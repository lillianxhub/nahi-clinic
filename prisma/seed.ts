...
async function main() {
    ...
    for (let day = 7; day >= 0; day--) {
        ...
        console.log(
            `📅 กำลังสร้างข้อมูลของวันที่: ${date.toLocaleDateString()}`,
        );
        ...
        const availableLots = await prisma.drug_Lot.findMany({
        ...
        for (let v = 0; v < dailyVisits; v++) {
            ...
            const selectedLot =
                availableLots[randomInt(0, availableLots.length - 1)];
            const qtyUsed = randomInt(1, 2);
            const drugInfo = drugs.find(
                (d) => d.drug_id === selectedLot.drug_id,
            );
