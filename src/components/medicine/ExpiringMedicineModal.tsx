...
export default function ExpiringMedicineModal({
    open,
    onClose,
    onRefresh,
}: Props) {
    ...
    const handleDiscard = async (lot: ExpiringLot) => {
        const confirm = await Swal.fire({
            ...
            html: `คุณกำลังทำลายยา <b>${lot.product.product_name}</b> ล็อต <b>${lot.lot_no}</b><br/>จำนวนคงเหลือ ${lot.qty_remaining} ${lot.product.unit}<br/><br/><span class="text-sm text-red-500">การกระทำนี้จะตัดยอดคงเหลือเป็น 0 และไม่สามารถแสดงยอดนี้ได้อีก</span>`,
            icon: "warning",
        ...
        if (confirm.isConfirmed) {
            try {
                await medicineService.discardDrugLot(
                    lot.lot_id,
                    "หมดอายุ/ทำลาย",
                );
                ...
                Swal.fire(
                    "เกิดข้อผิดพลาด",
                    error.message || "ไม่สามารถบันทึกการทำลายยาได้",
                    "error",
                );
            }
        }
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900">
                                จัดการยาหมดอายุ (ภายใน 30 วัน)
                            </h2>
                            <p className="text-sm text-gray-500">
                                ตรวจสอบและทำลายลดสต๊อกของยาที่หมดอายุ
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-auto p-6">
                    {loading ? (
                        ...
                        <div className="text-center py-10 text-gray-500">
                            ไม่มียาที่ใกล้หมดอายุใน 30 วัน
                        </div>
                    ) : (
                                    ...
                                    {lots.map((lot) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td
                                                    className={`px-4 py-3 font-medium ${isExpired ? "text-red-600" : "text-yellow-600"}`}
                                                >
                                                    {formatLocalDate(
                                                        new Date(
                                                            lot.expire_date,
