...
export async function PATCH(req: Request, { params }: Params) {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (existing.status === "completed") throw new Error("ไม่สามารถแก้ไขรายการที่เสร็จสิ้นแล้วได้");
            ...
            if (body.items && Array.isArray(body.items)) {
                ...
                for (const item of existing.items) {
                    ...
                    await tx.stockUsage.deleteMany({ where: { visit_item_id: item.visit_item_id } });
                    await tx.income.deleteMany({ where: { visit_item_id: item.visit_item_id } });
                }
                await tx.visitItem.deleteMany({ where: { visit_id: treatment_id } });
                ...
                for (const item of body.items) {
                    ...
                    itemsToProcess.push({ ...item, visit_item_id: visitItem.visit_item_id });
                }
            } else {
            ...
            if (existing.patient?.birth_date) {
                const age = calculateAge(new Date(existing.patient.birth_date), body.visit_date ? new Date(body.visit_date) : existing.visit_date);
            ...
            const updatedVisit = await tx.visit.update({
                ...
                data: {
                    ...
                    waistline: body.waistline !== undefined ? Number(body.waistline) : undefined,
            ...
            if (isCompletedNow && (isTransitioningToCompleted || (body.items && Array.isArray(body.items)))) {
                for (const item of itemsToProcess) {
                    ...
                    if (qty * price > 0) {
                        ...
                        await tx.income.create({
                            data: {
                                ...
                                amount: qty * price,
                                payment_method: (body.payment_method as any) || "cash",
                                receipt_no: body.receipt_no || generateReceiptNo("รักษา"),
                    ...
                    if (item.product_id) {
                        ...
                        if (product && (product.product_type === "drug" || product.product_type === "supply")) {
                            let remaining = qty;
                            const lots = await tx.inventoryLot.findMany({
                                where: { product_id: item.product_id, qty_remaining: { gt: 0 }, is_active: true, deleted_at: null },
                                orderBy: { expire_date: "asc" },
                            });
                            for (const lot of lots) {
                                if (remaining === 0) break;
                                const deduction = Math.min(lot.qty_remaining, remaining);
                                await tx.inventoryLot.update({ where: { lot_id: lot.lot_id }, data: { qty_remaining: { decrement: deduction } } });
                                await tx.stockUsage.create({ data: { visit_item_id: item.visit_item_id, lot_id: lot.lot_id, quantity: deduction, used_at: updatedVisit.visit_date } });
                                remaining -= deduction;
                            }
                            if (remaining > 0) throw new Error(`สินค้า ${item.product_id} ไม่เพียงพอ (เหลือ ${remaining})`);
                        }
                    }
                }
            }
...
export async function DELETE(_: Request, { params }: Params) {
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (!existing) throw new Error("ไม่พบข้อมูลการรักษา");
            if (existing.status === "completed") throw new Error("ไม่สามารถลบรายการที่เสร็จสิ้นแล้วได้");
            ...
            const now = new Date();
            await tx.visit.update({ where: { visit_id: treatment_id }, data: { deleted_at: now, is_active: false } });
            await tx.visitItem.updateMany({ where: { visit_id: treatment_id }, data: { deleted_at: now, is_active: false } });
            
            const itemIds = existing.items.map(i => i.visit_item_id);
            if (itemIds.length > 0) {
              await tx.stockUsage.updateMany({ where: { visit_item_id: { in: itemIds } }, data: { deleted_at: now } });
              await tx.income.updateMany({ where: { visit_item_id: { in: itemIds } }, data: { deleted_at: now } });
            }
